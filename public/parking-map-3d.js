...
(function () {
  ...
  function create(container, opts = {}) {
    ...
    const spotMeshes = [];
    function rowIdx(label) { return Math.max(0, (label || 'A').toUpperCase().charCodeAt(0) - 65); }
    // Strip non-digit chars so "A-01", "A-1", "A01", "A1" all parse to the same column.
    function colIdx(label) { const n = parseInt((label || '').replace(/\D/g, ''), 10); return Math.max(0, (isNaN(n) ? 1 : n) - 1); }
    ...
    function applyTheme() {
      ...
      if (light) {
        ...
        // Keep lamps dimly on even in day mode — active by default
        lampLights.forEach((l) => (l.intensity = l.distance > 8 ? 0.45 : 0.3));
      } else {
